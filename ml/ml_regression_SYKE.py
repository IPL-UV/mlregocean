...
#('S2ratio1',['S2ratio1']),('S2ratio2',['S2ratio2']),("ratios_S2",bands_S2ratio),
...
for name_bands, bands in bands_try:
    ...
    for name, model in models:
        ...
        joblib.dump(model, name+"_"+name_bands+'.pkl')
        ...
        # plt.show()
        fig.savefig("VALIDATION_" + name + "_" + name_bands + ".pdf")
        ...
        plt.xlabel("CDOM predicted")
        # ax.plot(ax.get_xlim(), ax.get_ylim(), ls="--", c=".3")
        ax.set_title("Residuals_" + name + "_" + name_bands)
        # plt.show()
        fig.savefig("RESIDUALS_" +  name + "_" + name_bands + ".pdf")
    ...
    dat_save.to_csv(name_bands + ".csv", index=False)
...
tabla_errores_totales.to_csv("tabla_errores_totales_SYSKE.csv", index=False)
